...
require_once("../../maestro_docente/model/UserModel.php");
...
function FancyTable($header, $data)
{
    ...
    foreach($data as $row)
    {
        $this->Cell($w[0],6,$row["cedula"],'LB',0,'C',$fill);
        $this->Cell($w[1],6,$row["nombre"]." ".$row["apellido"],'LB',0,'C',$fill);
        $this->Cell($w[2],6,$row["nombre_profesion"],'LB',0,'C',$fill);
        $this->Cell($w[3],6,$row["genero"],'LB',0,'C',$fill);
        $this->Cell($w[4],6,$row["fecha_nacimiento"],'LB',0,'C',$fill);
        $this->Cell($w[5],6,$row["rif"],'LB',0,'C',$fill);
...
$pdf->cell(0,10,"Listado De Tutores",0,1,"C");
$pdf->ln();
// Títulos de las columnas
$header = array("Cedula", "Nombre y Apellido", "Profesion", "Genero","Fecha Nacimiento","RIF","Direccion");
