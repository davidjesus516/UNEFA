    ...
    $('#formulario').submit(function(e){//reviso del formulario task el evento submit
        ...
        if (edit === false) {
            ...
            if(response==1){
                ...
                alert('Ya Este Periodo Existe Porfavor compruebe los Registros');
            } else {
                alert('?');
            }
            fetchTask();
...
$(document).on('click','.task-edit',function(){//escucho un click del boton task-edit que es una clase
        ...
        $.post('../controllers/periodo/UserEditSearch.php', {id}, function(response){//mando los datos al controlador
            const task = JSON.parse(response)[0]; // accede al primer objeto en el array
            $('#PERIOD_ID').val(task.PERIOD_ID).prop('readonly', true);//añado los elementos al formulario y lo hago de solo lectura
            $('#DESCRIPTION').val(task.DESCRIPTION).prop('readonly', true);
            $('#T_INTERNSHIPS_CODE').val(task.T_INTERNSHIPS_CODE);
