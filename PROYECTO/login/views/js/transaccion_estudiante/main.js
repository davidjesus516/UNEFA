$(document).ready(function(){//aqui inicializamos javascript
    ...
    $('#search').keyup(function(e){ //esta es una funcion q cada vez que en el campo #search se levante una tecla se realize una peticion
        ...
        if (/^\d{1,8}$/.test(search)) {//si el valor es un número entre 1 y 8 dígitos, realiza la búsqueda
            $.ajax({//realizamos una peticion ajax
                url: '../controllers/transaccion_estudiante/EstudianteList.php',// a esta url
                ...
                success: function(response){ //si recibo una respuesta exitosa haz esto
                    ...
                    datasearch.forEach(task =>{//itero el codigo con un foreach para que me traiga lo que encontro en el array
                        template +=`<option value="${task.id}">${task.cedula}-${task.nombre} ${task.apellido}</option>">`
                    })
    ...
    $('#task-form').submit(function(e){//reviso del formulario task el evento submit
            ...
            let url = '../controllers/transaccion_estudiante/UserAdd.php';
    ...
    function fetchTask(){//esta funcion es la que se encarga de traer todos los datos de la base de datos y los imprime en el html
        $.ajax({//realizo una peticion ajax
            url: '../controllers/transaccion_estudiante/UserList.php',//al url que trae la lista
    ...
    if (confirm('¿Estás seguro de eliminar este registro?')) {
        $.post('../controllers/transaccion_estudiante/UserDelete.php',{id}, function (response) {
            fetchTask();
        });
    }
});
...
$(document).on('click','.task-edit',function(){//escucho un click del boton task-edit que es una clase
        let element = $(this)[0].parentElement.parentElement;// accedo al elemento padre de este hasta conseguir el ID de la fila
        let id = $(element).attr('taskid');//accedo al tributo que cree que contiene la cedula que busco
        $.post('../controllers/transaccion_estudiante/UserEditSearch.php', {id}, function(response){//mando los datos al controlador
    ...
    $(document).on('click','.task-report',function(){
        ...
        $.post('../controllers/transaccion_estudiante/UserEditSearch.php', {id}, function(response){
    ...
    function combolapso(){//esta funcion es la que se encarga de traer todos los datos de la base de datos y los imprime en el html
        $.ajax({//realizo una peticion ajax
            url: '../controllers/transaccion_estudiante/LapsoList.php',//al url que trae la lista
            type: 'GET',//le pido una peticion GET
            success: function (response){// si tengo una respuesta ejecuta la funcion
                let task = JSON.parse(response);// convierto el json en string
    ...
    function comboempresa(){//esta funcion es la que se encarga de traer todos los datos de la base de datos y los imprime en el html
        $.ajax({//realizo una peticion ajax
            url: '../controllers/controllers_transaccion_estudiante/LapsoList.php',//al url que trae la lista
            type: 'GET',//le pido una peticion GET
            success: function (response){// si tengo una respuesta ejecuta la funcion
                let task = JSON.parse(response);// convierto el json en string
                let template = '<option value="" disabled="" selected="">Seleccione una opción</option>'; // Agregar la opción fuera del bucle
    ...
    $('#empresa').hide();//esto lo hice para que el buscar no se muestre mientras no exista una busqueda
    $('#empresaSearch').keyup(function(e){ //esta es una funcion q cada vez que en el campo #search se levante una tecla se realize una peticion
        let search = $('#empresaSearch').val();//leemos el valor que existe en el input
        if (/^\d{1,8}$/.test(search)) {//si el valor es un número entre 1 y 8 dígitos, realiza la búsqueda
            $.ajax({//realizamos una peticion ajax
                url: '../controllers/transaccion_estudiante/EmpresaList.php',// a esta url
                type: 'POST',//que envie los datos del input search cada vez que una tecla se presiona de forma dinamica
                data: {search},//enviamosla variable de tipo lectura search q fue declarada arriba
                success: function(response){ //si recibo una respuesta exitosa haz esto
                    let datasearch = JSON.parse(response);//primero convierte el json q recibi de php en un string 
                    let template = '<option value="" disabled="" selected="">Seleccione una opción</option>'; // Agregar la opción fuera del bucle
                    datasearch.forEach(task =>{//itero el codigo con un foreach para que me traiga lo que encontro en el array
                        template +=`<option value="${task.id}">${task.rif}-${task.nombre}-${task.direccion}</option>`
                    })
                    $('#empresa').html(template);//imprimo eso en el html
                    $('#empresa').show();// muestro los resultados
                }
            })
        } else {//si el valor no es un número entre 1 y 8 dígitos, oculta los resultados de búsqueda
            $('#empresa').hide();
    ...
    $('#docenteSearch').keyup(function(e){ //esta es una funcion q cada vez que en el campo #search se levante una tecla se realize una peticion
        ...
        if (/^\d{1,8}$/.test(search)) {//si el valor es un número entre 1 y 8 dígitos, realiza la búsqueda
            $.ajax({//realizamos una peticion ajax
                url: '../controllers/transaccion_estudiante/DocenteList.php',// a esta url
                type: 'POST',//que envie los datos del input search cada vez que una tecla se presiona de forma dinamica
                data: {search},//enviamosla variable de tipo lectura search q fue declarada arriba
                success: function(response){ //si recibo una respuesta exitosa haz esto
                    let datasearch = JSON.parse(response);//primero convierte el json q recibi de php en un string 
    ...
    $('#tutor').hide();//esto lo hice para que el buscar no se muestre mientras no exista una busqueda
    $('#tutorSearch').keyup(function(e){ //esta es una funcion q cada vez que en el campo #search se levante una tecla se realize una peticion
        let search = $('#tutorSearch').val();//leemos el valor que existe en el input
        if (/^\d{1,8}$/.test(search)) {//si el valor es un número entre 1 y 8 dígitos, realiza la búsqueda
            $.ajax({//realizamos una peticion ajax
                url: '../controllers/transaccion_estudiante/TutorList.php',// a esta url
                type: 'POST',//que envie los datos del input search cada vez que una tecla se presiona de forma dinamica
                data: {search},//enviamosla variable de tipo lectura search q fue declarada arriba
                success: function(response){ //si recibo una respuesta exitosa haz esto
                    let datasearch = JSON.parse(response);//primero convierte el json q recibi de php en un string 
                    ...
                    $('#tutor').html(template);//imprimo eso en el html
                    $('#tutor').show();// muestro los resultados
                }
            })
        } else {//si el valor no es un número entre 1 y 8 dígitos, oculta los resultados de búsqueda
            $('#tutor').hide();
    ...
    $('#filtrarDocente').change(function(e){
        let search = $('#filtrarDocente').val();
        $.ajax({
            url: '../controllers/transaccion_estudiante/UserSearch.php',
            type: 'POST',
            data: {search},
    ...
    $('#filtrarCarrera').change(function(e){
        let search = $('#filtrarCarrera').val();
        $.ajax({
            url: '../controllers/transaccion_estudiante/UserSearchCarrera.php',
            type: 'POST',
            data: {search},
            success: function(response){
                let datasearch = JSON.parse(response);
    ...
    $('#filtroTutor').change(function(e){
        let search = $('#filtroTutor').val();
        $.ajax({
            url: '../controllers/transaccion_estudiante/UserSearchTutor.php',
            type: 'POST',
            data: {search},
            success: function(response){
                let datasearch = JSON.parse(response);
    ...
    function combocarrera(){//esta funcion es la que se encarga de traer todos los datos de la base de datos y los imprime en el html
        $.ajax({//realizo una peticion ajax
            url: '../controllers/transaccion_estudiante/CarreraList.php',//al url que trae la lista
            type: 'GET',//le pido una peticion GET
            success: function (response){// si tengo una respuesta ejecuta la funcion
                let task = JSON.parse(response);// convierto el json en string
                let template = '<option value="" disabled="" selected="">Seleccione una opción</option>'; // Agregar la opción fuera del bucle
    ...
    function combodocente(){//esta funcion es la que se encarga de traer todos los datos de la base de datos y los imprime en el html
        $.ajax({//realizo una peticion ajax
            url: '../controllers/transaccion_estudiante/DocenteFilter.php',//al url que trae la lista
            type: 'GET',//le pido una peticion GET
            success: function (response){// si tengo una respuesta ejecuta la funcion
                let task = JSON.parse(response);// convierto el json en string
    ...
    function combotutor(){//esta funcion es la que se encarga de traer todos los datos de la base de datos y los imprime en el html
        $.ajax({//realizo una peticion ajax
            url: '../controllers/transaccion_estudiante/TutorFilter.php',//al url que trae la lista
            type: 'GET',//le pido una peticion GET
            success: function (response){// si tengo una respuesta ejecuta la funcion
                let task = JSON.parse(response);// convierto el json en string
                let template = '<option value="" selected="">Seleccione una opción</option>'; // Agregar la opción fuera del bucle
                task.forEach(task =>{//hago un array que me recorra el json y me lo imprima en el tbody
                    template +=`<option value="${task.id}">Tutor: ${task.nombre} ${task.apellido}</option>">`
                })
                $('#filtroTutor').html(template);//los imprimo en el html
            }
        }) 
    }
